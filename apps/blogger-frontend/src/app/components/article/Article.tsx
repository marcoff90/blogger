  ...
  return (
    <>
      <Box sx={{display: 'flex', flexDirection: 'row', alignItems: 'center', width: '60%'}} my={3}>
        <Box>
          <img width={'160px'} height={'160px'} src={article?.image} alt={'image'} style={{marginTop: '2rem', objectFit: 'cover', display: 'block'}}/>
        </Box>
        <Box sx={{display: 'flex', flexDirection: 'column'}} ml={5}>
          <Typography variant={'h5'} my={3}>{article.title}</Typography>
          <Box sx={{display: 'flex', flexDirection: 'row'}}>
            <Typography fontSize={10}>{article.username}&nbsp;•</Typography>
            <Typography
              fontSize={10}>&nbsp;{new Date(article.created_at * 1000).toLocaleDateString("en-US")}</Typography>
          </Box>
