...
export const createDatabase = (): void => {
  Postgres.createDatabase(RDS_USERNAME, RDS_PASSWORD, RDS_HOSTNAME, RDS_EXISTING_DB_NAME, RDS_DB_NAME_VOTES);
};

// then check if the database is created if so, sync model tables
export const createTables = (): void => {
  const client = Postgres.createClient(RDS_USERNAME, RDS_PASSWORD, RDS_HOSTNAME, RDS_DB_NAME_VOTES);
  client.connect(err => {
    if (err) {
      createTables();
    } else {
