...
import {
  ...
  CreateUserInput,
  ForgottenUserPasswordInput, IdentifyUserByResetTokenInput,
  LoginUserInput,
  ResetPasswordInput
...
const storeUser = async (req: Request<CreateUserInput['body']>,
...
const showLogin = async (req: Request<LoginUserInput['body']>,
                         res: Response, next: NextFunction) => {
  const loggedUser: UserI = await UserService.login(req.body);

  const token: string = loggedUser !== null ? await generateToken(loggedUser) : null;
...
const forgottenPassword = async (req: Request<ForgottenUserPasswordInput['body']>,
                                 res: Response, next: NextFunction) => {
...
const resetPassword = async (req: Request<ResetPasswordInput['body'],
  ResetPasswordInput['query']>, res: Response, next: NextFunction) => {
...
const activateAccount = async (req: Request<ActivateUserAccountInput['body'],
  ActivateUserAccountInput['query']>, res: Response, next: NextFunction) => {
  const timeNow = Date.now() / 1000;
  ...
  if (!user) {
    ...
    const newToken = await UserService.generateNewConfirmationToken(req.query.token.toString());
    if (newToken) {
...
const identifyUserByResetToken = async (req: Request<IdentifyUserByResetTokenInput['query']>,
                                        res: Response, next: NextFunction) => {
