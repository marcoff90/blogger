...
const create = async (user: UserI) => {
  ...
  log.info(`Created user ${savedUser.username} id: ${savedUser.id}`)
  return savedUser;
};
...
const confirmAccount = async (confirmationToken: string, avatar: string) => {
  ...
  await user.save();
  log.info(`Account confirmed for user ${user.username}`)
  return user;
};
...
const generateNewConfirmationToken = async (confirmationToken: string) => {
  ...
  await Mailer.sendConfirmationMail(user.email, token, user.username);
  await user.save();
  log.info(`Generated new confirmation token for user ${user.username}`);
  return token;
};

const login = async (user: UserI) => {
  const userInDb: UserI = await UserRepository.findByUsername(user.username);
  const doesPasswordMatch: boolean = userInDb && bcrypt.compareSync(user.password, userInDb.password);
  log.info(`Logging in user ${user.username}: result ${doesPasswordMatch}`);
...
const forgottenPassword = async (userEmail: string) => {
  ...
  log.info(`Generated reset password token for user ${user.username}`);
};
...
const resetPassword = async (userEmail: string, password: string) => {
  ...
  await user.save();
  log.info(`Reset password for user ${user.username}`);
};
